% =========================================================================
% teste_ris_fixa_cenarioB.m
% Teste SEM otimizacao no Cenario B (2 telas-obstaculo de 50 dB, sem parede).
% Avalia a vazao dos 96 usuarios (semente fixa, Monte Carlo R=20):
%   (1) SEM RIS  -> baseline (soma de potencias dos diretos Tx1+Tx2);
%   (2) COM RIS fixa em [x=40, parede Sul (y=0), z=1.5], para N=64 e N=256.
...
pos_RIS = [40, 0, 1.5];   % x=40 na parede Sul (y=0), altura z=1.5
sc = 1;                   % parede lateral (Sul) -> Scenario 1
N_list = [64, 256];

fprintf('=================================================================\n');
fprintf('TESTE FIXO CENARIO B (sem otimizacao) | %d usuarios | R_mc=%d\n', ...
        size(pos_Rx,1), cfg.R_mc);
fprintf('RIS fixa em [%.0f, %.0f, %.1f] (parede Sul)\n', pos_RIS);
fprintf('=================================================================\n');
...
save('teste_ris_fixa_cenarioB.mat', 'pos_RIS', 'sc', 'N_list', 'grupos', 'rotulos', 'resumo', 'cfg');
...
title({'Cenario B - Teste com RIS FIXA (sem otimizacao)', ...
       sprintf('RIS em [%.0f, %.0f, %.1f] (Sul) | obstaculos %d dB | %d usuarios', ...
               pos_RIS(1),pos_RIS(2),pos_RIS(3), cfg.att_obstaculo, size(pos_Rx,1))}, ...
      'FontSize',12);
legend(hM, {'Media'}, 'Location','northeast','FontSize',10);
grid on; box on;

fn = 'teste_ris_fixa_cenarioB.png';
